...
.. code-block:: python
    ...
    def test_operator_graceful_exiting():
        with KopfCLI(['run', '-m', 'myoperator'], exit_signal=signal.SIGTERM) as runner:
            time.sleep(3)
        assert runner.exit_code in (0, -signal.SIGTERM)

When :kwarg:`exit_signal` is set, the stop flag is not passed to the
...
If the process does not exit within :kwarg:`exit_timeout` (default
...
operator's reaction through the object state and :kwarg:`output`.
...
* **No detailed error propagation.** Exceptions raised by the operator
  ...
  is a generic non-zero :kwarg:`exit_code`; the full tracebacks are
  printed by the child's logging and available via :kwarg:`output`
...
* :kwarg:`exit_code` --- the child's exit code, or ``None`` while still
  running.
* :kwarg:`output` --- the accumulated stdout+stderr snapshot, readable
